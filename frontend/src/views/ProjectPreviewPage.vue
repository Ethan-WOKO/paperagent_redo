    ...
    <main class="project-workspace">
      ...
      <section v-else class="project-workspace__grid">
        ...
        <section class="project-panel project-panel--conversation">
          <div class="project-tabs"><div><button :class="{ active: centerTab === 'chat' }" @click="centerTab = 'chat'">Chat</button><button :class="{ active: centerTab === 'plan' }" @click="centerTab = 'plan'">Plan <span v-if="plans.length">{{ plans.length }}</span></button></div><NSpace :size="6"><NButton v-if="plans.length" size="tiny" quaternary @click="centerTab = centerTab === 'plan' ? 'chat' : 'plan'">{{ centerTab === 'plan' ? 'View current Chat' : 'View current Plan' }}</NButton><NButton size="tiny" quaternary :disabled="loading.send" @click="startNewConversation">New conversation</NButton></NSpace></div>
          <div class="project-conversation-history" aria-label="Project conversation history">
            <span>Conversations</span>
            <button v-for="session in projectSessions" :key="session.id" type="button" :class="{ active: session.id === activeSessionId }" :title="session.title" @click="selectConversation(session.id)">{{ session.title || `Conversation #${session.id}` }}</button>
          ...
          <template v-else>
            ...
            <div class="project-plans">
              <button v-for="plan in plans" :key="plan.id" class="project-plan" :class="{ active: selectedPlan?.id === plan.id }" @click="selectPlan(plan)"><div><strong>{{ plan.goal }}</strong><small>{{ plan.summary || 'No summary yet' }}</small></div><NTag size="small" :type="planTagType(planDisplayStatus(plan))">{{ planDisplayStatus(plan) }}</NTag><ul><li v-for="step in plan.steps" :key="step.id"><span>{{ step.sortOrder }}. {{ step.title || step.stepKey }}</span><NTag size="tiny" :type="planTagType(step.status)">{{ step.status }}</NTag><p v-if="step.errorMessage">{{ step.errorMessage }}</p><p v-else-if="step.result">{{ step.result }}</p></li></ul></button>
...
import { listMessages, listPlans, type AgentMessageResponse, type AgentPlanResponse, type AgentSessionResponse } from '@/api/agent';
import { getCandidateChange, listArtifacts, type CandidateChangeSet } from '@/api/artifact';
import { createProject, createProjectPlan, createProjectSession, deleteProject, getProjectManifest, listProjectEvidence, listProjectSessions, listProjects, readProjectFile, searchProject, sendProjectMessage, type ProjectEvidenceResponse, type ProjectFileResponse, type ProjectManifestResponse, type ProjectSearchHit, type ProjectSummaryResponse } from '@/api/project';
...
function planDisplayStatus(plan: AgentPlanResponse) { return plan.status.toUpperCase() === 'COMPLETED' && plan.steps.some((step) => ['DEGRADED', 'SKIPPED'].includes(step.status.toUpperCase())) ? 'PARTIAL' : plan.status; }
function planTagType(status: string): 'default' | 'success' | 'warning' | 'error' | 'info' { const value = status.toUpperCase(); if (value.includes('COMPLETED') || value.includes('VERIFIED')) return 'success'; if (value.includes('FAILED')) return 'error'; if (value.includes('PARTIAL') || value.includes('DEGRADED') || value.includes('SKIPPED') || value.includes('PENDING') || value.includes('RUNNING')) return 'warning'; return 'default'; }
...
function currentSessionId() { return activeSessionId.value; }
...
async function ensureSessionOnce(_recovered: boolean): Promise<number | null> { const project = activeProject.value; if (!project) return null; if (activeSessionId.value && projectSessions.value.some((item) => item.id === activeSessionId.value)) return activeSessionId.value; loading.sessions = true; try { projectSessions.value = (await listProjectSessions(project.id)).data; if (activeProjectId.value !== project.id) return null; if (projectSessions.value.length) { activeSessionId.value = projectSessions.value[0].id; return activeSessionId.value; } const created = (await createProjectSession(project.id, { title: project.name, ragDisabled: true })).data; if (activeProjectId.value !== project.id) return null; projectSessions.value = [created]; activeSessionId.value = created.id; return created.id; } finally { if (activeProjectId.value === project.id) loading.sessions = false; } }
...
async function selectProject(projectId: number) { closeProjectSocket(); currentAssistantMessageId = null; currentProcessMessageId = null; projectEpoch++; sessionFlight = null; if (planPoll != null) { window.clearTimeout(planPoll); planPoll = null; } loading.file = false; loading.search = false; loading.send = false; activeProjectId.value = projectId; activeSessionId.value = null; projectSessions.value = []; collapsedDirectories.value = new Set(collapsedDirectoriesByProject.get(projectId) || []); manifest.value = null; selectedFile.value = null; searchResults.value = []; messages.value = []; plans.value = []; evidence.value = []; candidates.value = []; selectedPlan.value = null; selectedCandidate.value = null; const epoch = projectEpoch; await Promise.all([loadManifest(epoch), loadConversation(epoch)]); }
...
async function selectConversation(sessionId: number) { if (sessionId === activeSessionId.value || loading.send) return; closeProjectSocket(); currentAssistantMessageId = null; currentProcessMessageId = null; projectEpoch++; sessionFlight = null; activeSessionId.value = sessionId; messages.value = []; plans.value = []; evidence.value = []; candidates.value = []; selectedPlan.value = null; selectedCandidate.value = null; const epoch = projectEpoch; loading.messages = true; loading.plans = true; try { await Promise.all([loadMessages(sessionId, epoch), loadPlans(sessionId, epoch), loadCandidates(sessionId, epoch)]); } catch (cause) { if (epoch === projectEpoch) error.value = apiError(cause); } finally { if (epoch === projectEpoch) { loading.messages = false; loading.plans = false; } } }
async function startNewConversation() { const project = activeProject.value; if (!project || loading.send) return; closeProjectSocket(); currentAssistantMessageId = null; currentProcessMessageId = null; projectEpoch++; sessionFlight = null; messages.value = []; plans.value = []; evidence.value = []; candidates.value = []; selectedPlan.value = null; selectedCandidate.value = null; const epoch = projectEpoch; loading.sessions = true; try { const created = (await createProjectSession(project.id, { title: project.name, ragDisabled: true })).data; if (epoch !== projectEpoch) return; projectSessions.value = [created, ...projectSessions.value.filter((item) => item.id !== created.id)]; activeSessionId.value = created.id; centerTab.value = 'chat'; } catch (cause) { if (epoch === projectEpoch) error.value = apiError(cause); } finally { if (epoch === projectEpoch) loading.sessions = false; } }
async function removeActiveProject() {
  ...
  try {
    ...
    manifest.value = null; selectedFile.value = null; searchResults.value = []; projectSessions.value = []; activeSessionId.value = null; messages.value = []; plans.value = []; evidence.value = []; candidates.value = []; selectedPlan.value = null; selectedCandidate.value = null; collapsedDirectories.value = new Set();
