...
public final class SandboxCommandProfiles {
    ...
    private static boolean runner(List<String> argv){
        ...
        return switch(argv.get(1)){
            case "java" -> source(argv.get(2));
            case "python" -> source(argv.get(2),".py");
            case "c" -> source(argv.get(2),".c");
            case "cpp" -> source(argv.get(2),".cc")||source(argv.get(2),".cpp")||source(argv.get(2),".cxx");
            default -> false;
        };
    }
