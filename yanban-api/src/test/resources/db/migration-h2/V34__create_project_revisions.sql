...
CREATE INDEX idx_project_revisions_owner_history ON project_revisions(user_id, project_id, created_at, id);
...
CREATE TABLE project_revision_operations (
    ...
    accepted_change_indexes CLOB NOT NULL,
    rejected_change_indexes CLOB NOT NULL,
...
CREATE INDEX idx_project_revision_operations_audit ON project_revision_operations(user_id, project_id, created_at, id);

ALTER TABLE projects ADD COLUMN current_revision_id BIGINT NULL;
ALTER TABLE projects ADD COLUMN revision_lock BIGINT NOT NULL DEFAULT 0;
