...
class SandboxOutboxExecution {
    ...
    void cancelBeforeDispatch(LocalDateTime now){if(receiptDigest==null&&brokerExecutionId==null&&!java.util.Set.of("SUCCEEDED","FAILED","CANCELLED","TIMED_OUT","CLEANUP_FAILED").contains(status)){status="CANCELLED";errorCode="USER_CANCELLED";requestJson=null;nextAttemptAt=null;updatedAt=now;releaseClaim();}}
    boolean reconcileable(LocalDateTime now){return java.util.Set.of("PENDING","RETRY","API_CANCEL_REQUESTED","ACCEPTED","CLAIMED","MATERIALIZING","CREATED","POLICY_APPLIED","RUNNING","SUCCEEDED_PENDING_CLEANUP","FAILED_PENDING_CLEANUP","CANCEL_REQUESTED","TIMED_OUT_PENDING_CLEANUP","CLEANING","RECEIPT_PENDING_PROJECTION").contains(status)&&(nextAttemptAt==null||!nextAttemptAt.isAfter(now));}
