...
public class BrokerProperties {
    ...
    public boolean isExecutableSafe(){if(!enabled)return true;try{var p=java.nio.file.Path.of(sbxExecutable);return p.isAbsolute()&&p.getNameCount()>1;}catch(Exception e){return false;}}
    ...
    public boolean isProcessIdentitySafe(){if(!enabled)return true;if(mode==Mode.LOCAL_ACCEPTANCE)return System.getProperty("os.name","").toLowerCase(java.util.Locale.ROOT).contains("windows")&&!remoteAccess&&java.util.Set.of("127.0.0.1","::1","localhost").contains(bindAddress);return System.getProperty("os.name","").toLowerCase(java.util.Locale.ROOT).contains("linux")
            && !"root".equals(System.getProperty("user.name")) && expectedUser.equals(System.getProperty("user.name"))&&java.nio.file.Files.isReadable(java.nio.file.Path.of("/dev/kvm"))&&java.nio.file.Files.isWritable(java.nio.file.Path.of("/dev/kvm"));}
    ...
    public boolean isProviderEnvironmentSafe(){if(!enabled)return true;return safeDirectory(providerHome)&&safeDirectory(providerConfigHome)&&safeDirectory(providerDataHome)&&safeDirectory(providerStateHome);}
